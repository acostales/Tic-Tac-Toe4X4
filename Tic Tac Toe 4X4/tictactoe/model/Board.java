...
public class Board 
{
    ...
    public int[] evaluateBoard() 
    {
        ...
        // evaluate the fourth row
        for (int i = 0; i < BOARD_SIDE_LENGTH; i++)
        {
            if (gameGrid[3][i].getOwner() == Symbol.X) scores[3]++;
            if (gameGrid[3][i].getOwner() == Symbol.O) scores[3]--;
        }

        // evaluate the first column
        for (int i = 0; i < BOARD_SIDE_LENGTH; i++) 
        {
            if (gameGrid[i][0].getOwner() == Symbol.X) scores[4]++;
            if (gameGrid[i][0].getOwner() == Symbol.O) scores[4]--;
        }

        // evaluate the second column
        for (int i = 0; i < BOARD_SIDE_LENGTH; i++) 
        {
            if (gameGrid[i][1].getOwner() == Symbol.X) scores[5]++;
            if (gameGrid[i][1].getOwner() == Symbol.O) scores[5]--;
        }

        // evaluate the third column
        for (int i = 0; i < BOARD_SIDE_LENGTH; i++) 
        {
            if (gameGrid[i][2].getOwner() == Symbol.X) scores[6]++;
            if (gameGrid[i][2].getOwner() == Symbol.O) scores[6]--;
        }
        
        // evaluate the fourth column
        for (int i = 0; i < BOARD_SIDE_LENGTH; i++) 
        {
        ...
        for (int i = 0; i < BOARD_SIDE_LENGTH; i++) 
        {
            if (gameGrid[i][i].getOwner() == Symbol.X) scores[8]++;
            if (gameGrid[i][i].getOwner() == Symbol.O) scores[8]--;
        }

        // evaluate the right-to-left diagonal
        for (int i = 0; i < BOARD_SIDE_LENGTH; i++) //the row
        { 
            for (int j = 0; j < BOARD_SIDE_LENGTH; j++) //the column
            { 
                if (i + j == 2)
                { 
                    if (gameGrid[i][j].getOwner() == Symbol.X) scores[9]++;
                    if (gameGrid[i][j].getOwner() == Symbol.O) scores[9]--;
                }
            }
        }
